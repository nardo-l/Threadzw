...
// These are the public object URLs for the uploaded preview images.
// Keep the bucket path explicitly encoded so the space in "sample themes"
// cannot be altered by URL construction or storage helpers.
        ...
        >
          <div className="mx-auto flex h-full w-full max-w-[520px] flex-col">
            <header className="relative flex shrink-0 items-center justify-between px-4 pb-3 pt-4 sm:px-6 sm:pt-5">
              <button type="button" onClick={() => setOpen(false)} className="flex h-11 w-11 items-center justify-center rounded-2xl bg-white text-[#10204b] shadow-[0_5px_18px_rgba(54,88,150,.12)] ring-1 ring-[#dfe8fa]" aria-label="Close theme picker"><ArrowLeft size={21} strokeWidth={2.2} /></button>
            ...
            <div className="shrink-0 px-5 pt-1 sm:px-7 sm:pt-2">
              ...
              <div className="flex gap-2" aria-label="Theme progress">
                {STOREFRONT_THEMES.map(theme => <button key={theme.id} type="button" onClick={() => scrollToIndex(STOREFRONT_THEMES.findIndex(item => item.id === theme.id))} aria-label={`View ${theme.name}`} className="h-1.5 flex-1 rounded-full transition-all duration-200" style={{ background: selectedId === theme.id ? theme.accent : '#c6d3eb', transform: selectedId === theme.id ? 'scaleY(1.35)' : undefined }} />)}
              </div>
            </div>
            ...
            <div ref={carouselRef} onScroll={handleCarouselScroll} className="mt-3 flex min-h-0 flex-1 snap-x snap-mandatory items-center gap-4 overflow-x-auto overscroll-x-contain px-[11%] py-2 scrollbar-hide" style={{ WebkitOverflowScrolling: 'touch', scrollPaddingInline: '11%' }}>
              {STOREFRONT_THEMES.map((theme, index) => {
                const imageUrl = getThemeImageUrl(theme.id);
                ...
                return (
                  <section key={theme.id} data-theme-index={index} className="flex min-w-[78%] snap-center flex-col items-center justify-center">
                    <button
                      ...
                      style={{ border: `2px solid ${isSelected ? theme.accent : '#d6e0f2'}`, boxShadow: isSelected ? `0 18px 45px ${theme.accent}30, 0 5px 16px rgba(45,72,120,.10)` : '0 10px 28px rgba(45,72,120,.10)' }}
                    >
                      {!imageFailed ? (
                        <img
                          src={imageUrl}
            ...
            <div className="shrink-0 px-5 pb-[max(18px,env(safe-area-inset-bottom))] pt-1 sm:px-7 sm:pb-6">
              ...
              <div className="mb-3 flex items-center justify-between gap-3">
                <button type="button" onClick={() => scrollToIndex(selectedIndex - 1)} disabled={selectedIndex === 0} className="flex h-11 shrink-0 items-center gap-2 rounded-2xl bg-white px-4 text-xs font-bold text-[#35558f] shadow-[0_5px_18px_rgba(54,88,150,.10)] ring-1 ring-[#dfe8fa] disabled:cursor-not-allowed disabled:opacity-40" aria-label="Previous theme"><ChevronLeft size={18} /><span className="hidden sm:inline">Previous</span></button>
                <p className="text-center text-[9px] font-semibold text-[#7283a8] sm:text-[10px]">Swipe left or right to preview themes</p>
                <button type="button" onClick={() => scrollToIndex(selectedIndex + 1)} disabled={selectedIndex === STOREFRONT_THEMES.length - 1} className="flex h-11 shrink-0 items-center gap-2 rounded-2xl bg-white px-4 text-xs font-bold text-[#35558f] shadow-[0_5px_18px_rgba(54,88,150,.10)] ring-1 ring-[#dfe8fa] disabled:cursor-not-allowed disabled:opacity-40" aria-label="Next theme"><span className="hidden sm:inline">Next</span><ChevronRight size={18} /></button>
              </div>
