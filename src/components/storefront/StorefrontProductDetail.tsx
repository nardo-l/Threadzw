...
export const StorefrontProductDetail: React.FC<StorefrontProductDetailProps> = ({
  ...
  const { getProductRating, getShopRating, reviews, addReview, voteReview } = useInventory();
  ...
  const [activeReviewPhoto, setActiveReviewPhoto] = useState<string | null>(null);

  // Load product specific reviews dynamically
  const productReviews = useMemo(() => {
    const list = reviews[shop.id] || [];
    return list.filter((r: any) => r.product_id === product.id);
  }, [reviews, shop.id, product.id]);

  // Check if current visitor has logged an intent for this product
  const hasActiveIntent = useMemo(() => {
    try {
      const cached = localStorage.getItem('threadzw_buyer_intents_logged');
      const list = cached ? JSON.parse(cached) : [];
      return list.some((i: any) => i.productId === product.id);
    } catch (e) {
      return false;
    }
  }, [product.id]);
  ...
  return (
    <div className="space-y-6 px-5 pb-44 select-none text-left bg-white min-h-screen pt-4 font-sans">
      ...
      <div className="space-y-1.5 text-left">
        ...
        >
          <div className="flex items-center gap-0.5 text-amber-500">
            {[1, 2, 3, 4, 5].map((s) => (
              <Star key={s} size={12} className={s <= Math.round(Number(getProductRating(product.id).score)) ? "fill-amber-500 text-amber-500" : "text-zinc-200"} />
            ))}
          </div>
          <span className="text-xs font-bold text-zinc-700 ml-0.5">
            {getProductRating(product.id).score}
          </span>
          <span className="text-[10px] text-zinc-400 font-medium">
            ({getProductRating(product.id).count} reviews)
          </span>
        </div>
      ...
      {/* ----------------- 6.5 PRODUCT REVIEWS & RATINGS SECTION ----------------- */}
      <div id="product-reviews-section" className="pt-8 border-t border-zinc-100 space-y-6 text-left">
        <div className="flex items-center justify-between">
          <h3 className="font-bold text-base text-zinc-900 font-sans tracking-tight">Customer Reviews</h3>
          <span className="text-xs font-bold text-zinc-400 uppercase font-sans">
            {productReviews.length} Reviews
          </span>
        </div>

        {/* 1. Star Rating Summary & Breakdown Card */}
        <div className="bg-zinc-50 border border-zinc-150 rounded-2xl p-5 grid grid-cols-1 md:grid-cols-12 gap-5">
          {/* Main Average Column */}
          <div className="md:col-span-5 flex flex-col items-center justify-center border-b md:border-b-0 md:border-r border-zinc-200/60 pb-4 md:pb-0 md:pr-4 text-center select-none">
            <span className="text-4xl font-extrabold text-zinc-900 font-sans">
              {getProductRating(product.id).score}
            </span>
            <div className="flex gap-0.5 text-amber-500 my-1.5">
              {[1, 2, 3, 4, 5].map((s) => (
                <Star key={`summary-star-${s}`} size={14} className={s <= Math.round(Number(getProductRating(product.id).score)) ? "fill-amber-500" : "text-zinc-200"} />
              ))}
            </div>
            <span className="text-[10px] text-zinc-400 font-medium font-sans">
              Based on {getProductRating(product.id).count} community ratings
            </span>
          </div>

          {/* Dynamic Breakdown Bars Column */}
          <div className="md:col-span-7 space-y-2 select-none">
            {[5, 4, 3, 2, 1].map((ratingVal) => {
              const matchingCount = productReviews.filter((r: any) => r.rating === ratingVal).length;
              const totalCount = productReviews.length || 1;
              const percentage = Math.round((matchingCount / totalCount) * 100);
              
              return (
                <div key={`breakdown-row-${ratingVal}`} className="flex items-center gap-2.5 text-xs font-sans">
                  <span className="w-3 font-semibold text-zinc-500 text-right">{ratingVal}</span>
                  <Star size={10} className="text-amber-400 fill-amber-400 shrink-0" />
                  <div className="flex-1 h-2 bg-zinc-200/70 rounded-full overflow-hidden">
                    <div 
                      className="h-full bg-green-600 rounded-full transition-all duration-500"
                      style={{ width: `${percentage}%` }}
                    />
                  </div>
                  <span className="w-7 text-right font-medium text-zinc-400 text-[10px]">
                    {percentage}%
                  </span>
                </div>
              );
        ...
        {/* 2. AI Product Insight Summary Banner */}
        <div className="bg-emerald-50/50 border border-emerald-100/50 p-4 rounded-2xl flex items-start gap-3">
          <span className="text-lg leading-none mt-0.5 shrink-0 select-none">✨</span>
          <div className="space-y-0.5">
            <h5 className="text-[10px] uppercase font-extrabold text-emerald-800 tracking-wider font-sans">AI Product Insight</h5>
            <p className="text-xs text-zinc-600 font-medium leading-relaxed font-sans">
              {Number(getProductRating(product.id).score) >= 4.5 
                ? "Highly recommended streetwear! Customers consistently praise the tailored fit, premium stitching, and true-to-size cut of this piece."
                : "A popular casual piece. Customers mention high visual satisfaction but recommend paying close attention to size measurements to guarantee the perfect fit."}
            </p>
          </div>
        </div>

        {/* 3. Verified Buyer Intent / Qualification Info */}
        <div className="p-4 rounded-2xl border flex items-start gap-3 bg-zinc-50 border-zinc-150">
          <span className="text-base shrink-0 leading-none select-none">
            {hasActiveIntent ? '✅' : '💡'}
          </span>
          <div className="space-y-0.5">
            <h5 className="text-[10px] uppercase font-extrabold text-zinc-700 tracking-wider font-sans">
              {hasActiveIntent ? 'Verified Buyer Intent Unlocked' : 'Verified Reviewer Program'}
            </h5>
            <p className="text-xs text-zinc-500 leading-relaxed font-sans">
              {hasActiveIntent 
                ? 'Excellent! You recently expressed interest in ordering this product, which qualifies your review for the exclusive ✅ Verified Buyer Intent badge.'
                : 'Reviews with a Verified badge carry 3x more weight. Tap "Order on WhatsApp" or "Buy Now" to verify your active intent.'}
            </p>
          </div>
        </div>
        ...
        <div className="bg-white border border-zinc-150 p-5 rounded-2xl space-y-4">
          <h4 className="text-xs font-bold text-zinc-800 uppercase tracking-wide font-sans">Write a Review</h4>
          ...
          <form 
            onSubmit={(e) => {
              ...
              // Auto badging based on criteria
              const badgesList = [];
              if (productReviews.length < 3) {
                badgesList.push("🌟 Early Reviewer");
              } else if (reviewRating === 5) {
                badgesList.push("🏆 Style Critic");
              } else {
                badgesList.push("💎 Top Reviewer");
              }
              ...
              addReview(shop.id, {
                ...
                userHandle: reviewerHandle.trim() || `@${reviewerName.toLowerCase().replace(/\s+/g, '_')}`,
                isVerified: hasActiveIntent,
                badges: badgesList,
                images: reviewPhotos
              });

              toast.success("Review submitted! Thank you for supporting local brand credibility.");
          ...
          >
            ...
            <div className="space-y-1.5 text-left">
              <span className="block font-bold text-zinc-700 font-sans">Your Rating</span>
            ...
            {/* Critic Identification inputs */}
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                <label className="block font-bold text-zinc-700 font-sans">Full Name</label>
                <input 
                  type="text" 
                  required
                  ...
                  className="w-full p-2.5 bg-zinc-50 border border-zinc-200 rounded-xl focus:border-green-500 focus:bg-white outline-none transition-all font-sans font-medium"
                />
              </div>
              <div className="space-y-1">
                <label className="block font-bold text-zinc-700 font-sans">Social Handle (Optional)</label>
                <input 
                  type="text" 
                  placeholder="e.g., @sarah_thrift"
            ...
            {/* Critique Comment Textarea */}
            <div className="space-y-1">
              <label className="block font-bold text-zinc-700 font-sans font-sans">Review Comments</label>
              <textarea 
                rows={3}
                required
                placeholder="Share your thoughts on fabric weight, stitching, color vibrancy, or size correctness..."
                value={reviewText}
                onChange={(e) => setReviewText(e.target.value)}
                className="w-full p-3 bg-zinc-50 border border-zinc-200 rounded-xl focus:border-green-500 focus:bg-white outline-none transition-all font-sans font-medium resize-none leading-relaxed"
              />
            </div>

            {/* Photo Drag & Drop Upload field */}
            <div className="space-y-1.5">
              <span className="block font-bold text-zinc-700 font-sans">Upload Outfit Photos (Max 3)</span>
              ...
              <div 
                ...
                onDrop={(e) => {
                  ...
                  if (e.dataTransfer.files) {
                    ...
                    if (reviewPhotos.length + filesList.length > 3) {
                      toast.error("Maximum 3 review photos allowed");
                      return;
                    }
                ...
                className={`border-2 border-dashed rounded-2xl p-5 text-center transition-all cursor-pointer flex flex-col items-center justify-center gap-1 bg-zinc-50/50 ${
                  dragActive ? 'border-green-500 bg-green-50/20' : 'border-zinc-200 hover:border-zinc-300'
                }`}
                onClick={() => document.getElementById('review-img-upload')?.click()}
              >
                <Camera className="w-5 h-5 text-zinc-400 shrink-0" />
                <span className="font-semibold text-zinc-600 block mt-1 font-sans">Drag and drop outfits or browse</span>
                <span className="text-[10px] text-zinc-400 font-sans">PNG, JPG up to 5MB</span>
                <input 
              ...
              {reviewPhotos.length > 0 && (
                <div className="flex gap-2.5 mt-2.5">
                  {reviewPhotos.map((url, idx) => (
                    <div key={`form-photo-idx-${idx}`} className="relative w-14 h-14 rounded-xl border border-zinc-200 overflow-hidden shrink-0">
                      <img src={url} alt="Review thumb" referrerPolicy="no-referrer" className="w-full h-full object-cover" />
                      <button
                        type="button"
                        onClick={(e) => {
                          e.stopPropagation();
            ...
            <button
              ...
              className="w-full py-2.5 bg-zinc-950 hover:bg-black text-[#C6FF00] hover:text-white font-bold rounded-xl transition-all uppercase tracking-wider text-[11px] disabled:opacity-50 cursor-pointer"
            >
              {isSubmittingReview ? 'Publishing...' : 'Publish Critique'}
            </button>
          </form>
        </div>

        {/* 5. Reviews List Display */}
        <div className="space-y-4">
          {productReviews.length === 0 ? (
            <div className="text-center py-8 bg-zinc-50 border border-zinc-150 rounded-2xl select-none">
              <span className="text-xl block leading-none">👗</span>
              <p className="text-xs font-semibold text-zinc-500 mt-2 font-sans">No reviews published yet for this piece</p>
              <p className="text-[10px] text-zinc-400 mt-1 font-sans">Be the very first early reviewer to share feedback!</p>
            </div>
          ) : (
            productReviews.map((rev: any, idx: number) => {
              const reviewId = rev.id || `p-review-${idx}`;
              return (
                <div 
                  key={reviewId}
                  className="bg-zinc-50/50 border border-zinc-150 rounded-2xl p-4 space-y-3 relative"
                >
                  {/* Header metadata row */}
                  <div className="flex items-start justify-between">
                    <div>
                      <div className="flex items-center gap-1.5 flex-wrap">
                        <span className="font-bold text-zinc-800 text-xs font-sans">{rev.userName}</span>
                        <span className="text-[10px] text-zinc-400 font-sans font-medium">{rev.userHandle}</span>
                        
                        {/* Awarded Badges */}
                        {rev.badges && rev.badges.map((b: string, bidx: number) => (
                          <span 
                            key={`badge-row-${bidx}`} 
                            className="text-[9px] font-bold bg-green-50 text-green-700 border border-green-100/50 px-1.5 py-0.5 rounded-md font-sans shrink-0"
                          >
                            {b}
                          </span>
                        ))}
                      ...
                      <div className="flex items-center gap-1.5 mt-1 flex-wrap">
                        ...
                        <div className="flex text-amber-500">
                          {[1, 2, 3, 4, 5].map((s) => (
                            <Star key={`rev-star-${reviewId}-${s}`} size={11} className={s <= rev.rating ? "fill-amber-500" : "text-zinc-200"} />
                          ))}
                        </div>
                        
                        {/* Verified Buyer Intent Badge */}
                        {rev.isVerified && (
                          <span className="text-[9px] font-extrabold text-emerald-700 uppercase font-sans flex items-center gap-0.5 bg-emerald-50 px-1.5 py-0.5 rounded-md">
                    ...
                    <span className="text-[10px] text-zinc-400 font-mono font-bold">
                      {rev.created_at ? new Date(rev.created_at).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' }) : 'July 2, 2026'}
                    </span>
                  </div>

                  {/* Comments Content */}
                  <p className="text-zinc-700 text-xs font-sans font-medium leading-relaxed font-sans whitespace-pre-line">
                    {rev.text}
                  </p>
                  ...
                  {rev.images && rev.images.length > 0 && (
                    <div className="flex gap-2 pt-1 flex-wrap">
                      {rev.images.map((imgUrl: string, pidx: number) => (
                        <div 
                          key={`review-p-idx-${reviewId}-${pidx}`} 
                          onClick={() => setActiveReviewPhoto(imgUrl)}
                          className="w-20 h-20 rounded-xl border border-zinc-200 overflow-hidden shrink-0 cursor-pointer hover:opacity-90 active:scale-95 transition-all"
                        >
                          <img src={imgUrl} alt="Review Outfit" referrerPolicy="no-referrer" className="w-full h-full object-cover" />
                        </div>
                      ))}
                    </div>
                  )}
                  ...
                  <div className="flex items-center gap-3 pt-1 select-none">
                    <button
                      onClick={() => {
                        voteReview(shop.id, reviewId, 'helpful');
                        toast.success("Thank you for voting!");
                      }}
                      className="text-[10px] text-zinc-500 font-bold font-sans hover:text-green-600 active:scale-95 transition-transform flex items-center gap-1 cursor-pointer bg-zinc-100 px-2 py-1 rounded-lg"
                    >
                      <ThumbsUp size={11} /> Helpful ({rev.helpfulCount || 0})
                    </button>
                    <button
                      onClick={() => {
                        voteReview(shop.id, reviewId, 'unhelpful');
                        toast.success("Vote registered!");
                      }}
                      className="text-[10px] text-zinc-400 font-bold font-sans hover:text-red-500 active:scale-95 transition-transform flex items-center gap-1 cursor-pointer bg-zinc-100/50 px-2 py-1 rounded-lg"
                    >
                      <ThumbsDown size={11} /> Unhelpful ({rev.unhelpfulCount || 0})
                    </button>
                  </div>

                  {/* Merchant Reply bubble */}
                  {(rev.reply || rev.sellerResponse || rev.seller_response) && (
                    <div className="mt-3 bg-zinc-100/80 border border-zinc-200/50 p-3 rounded-2xl flex items-start gap-2.5 text-left text-xs font-sans font-medium">
                      <div className="w-6 h-6 rounded-full border border-zinc-200 overflow-hidden shrink-0 shadow-2xs">
                        <ShopLogo shop={shop} size="100%" className="w-full h-full rounded-full object-cover" />
                      </div>
                      <div className="space-y-0.5">
                        <div className="flex items-center gap-1.5">
                          <span className="font-extrabold text-zinc-800 text-[11px] font-sans">Store Reply</span>
                          <span className="text-[9px] font-bold bg-zinc-200 text-zinc-600 px-1 py-0.2 rounded-md font-sans">Seller</span>
                        </div>
                        <p className="text-zinc-600 text-xs leading-relaxed font-sans whitespace-pre-line leading-relaxed">
                          {typeof (rev.sellerResponse || rev.reply || rev.seller_response) === 'object' 
                            ? (rev.sellerResponse?.text || JSON.stringify(rev.sellerResponse)) 
                            : (rev.reply || rev.seller_response)}
                        </p>
                      </div>
                    </div>
                  )}
      ...
      <AnimatePresence>
        {activeReviewPhoto && (
          <motion.div
            ...
            className="fixed inset-0 bg-black/95 z-[110] flex flex-col items-center justify-center p-6 select-none backdrop-blur-md cursor-pointer"
          >
            ...
            <div className="absolute top-6 right-6 z-[120]">
              <button
                ...
                className="p-3 bg-white/10 hover:bg-white/20 rounded-full text-white cursor-pointer border border-white/10 transition-colors"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
