...
export const Shops: React.FC = () => {
  ...
  if (loading && !shops.length) {
    return (
      <div className="flex flex-col gap-8 p-6 min-h-screen bg-[#0d0d0d]">
        ...
        >
          <h1 className="text-3xl font-pacifico text-[#FF2D78]">Shops</h1>
        </header>
  ...
  return (
    <div className="flex flex-col gap-8 p-6 min-h-screen bg-[#0d0d0d]">
      ...
      >
        <h1 className="text-3xl font-pacifico text-[#FF2D78]">Shops</h1>
        <button className="p-2 rounded-full bg-[#111] text-[#FF2D78]"><Search size={20} /></button>
      </header>
      ...
      <div className="relative">
        <input 
          ...
          className="w-full border rounded-pill py-4 pl-14 pr-6 placeholder:text-muted focus:ring-2 transition-all outline-none bg-[#111] border-[#222] text-white focus:ring-[#FF2D78]"
        />
        <Search className="absolute left-6 top-1/2 -translate-y-1/2 text-[#FF2D78]" size={20} />
      </div>

      {/* Affiliate Banner */}
      <div className="rounded-card p-6 flex flex-col gap-4 relative overflow-hidden bg-gradient-to-br from-[#FF2D78] to-[#FF2D78]">
        ...
        <button 
          ...
          className="bg-white font-bold py-3 rounded-pill text-sm hover:bg-opacity-90 transition-all active:scale-[0.98] text-[#FF2D78]"
        >
          Start Earning
        </button>
      </div>
