  ...
  return (
    <div className="min-h-screen bg-[#F8F9FA] p-4 sm:p-8 font-sans">
      <div className="w-full max-w-xl mx-auto">
        ...
        {pending && !pro && <div className="bg-amber-50 border border-amber-200 rounded-3xl p-5 mb-5"><div className="flex gap-3"><Clock className="text-amber-600 shrink-0" size={20} /><div><h2 className="text-sm font-black text-amber-950">PAYMENT AWAITING APPROVAL</h2><p className="text-xs text-amber-800 mt-1 leading-relaxed">Your shop is pending payment verification. You can add up to <strong>9 products</strong> while the ThreadZW team checks your NardoPay payment.</p><button onClick={refresh} className="mt-3 inline-flex items-center gap-2 px-3 py-2 rounded-xl bg-amber-600 text-white text-[11px] font-bold"><RefreshCw size={13} /> Refresh status</button></div></div></div>}
        ...
        <p className="text-center text-[11px] text-zinc-500 mt-5">Payments are processed securely by NardoPay. ThreadZW activates Premium only after the payment is approved.</p>
      </div>
    </div>
  );
};
