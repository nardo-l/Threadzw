...
function mix(first: Rgb, second: Rgb, weight: number): Rgb {
  return { r: clamp(first.r * (1 - weight) + second.r * weight), g: clamp(first.g * (1 - weight) + second.g * weight), b: clamp(first.b * (1 - weight) + second.b * weight) };
}
...
function logoAccentTheme(imageData: ImageData): StorefrontTheme {
  ...
  for (let i = 0; i < imageData.data.length; i += 16) {
    ...
    const bucket = { r: Math.round(color.r / 16) * 16, g: Math.round(color.g / 16) * 16, b: Math.round(color.b / 16) * 16 };
...
function applyThemeStyles(theme: StorefrontTheme) {
  ...
  style.textContent = `
    ...
    .storefront-root .bg-rose-500 { background:${theme.accent} !important; }
    .storefront-root .text-rose-500, .storefront-root .text-red-600 { color:${theme.accent} !important; }
    .storefront-root .bg-red-500 { background:${theme.accent} !important; }
    ...
    ${dark ? `
      .storefront-root .bg-black\/10 { background:rgba(0,0,0,.28) !important; }
      .storefront-root .bg-zinc-50\/50 { background:${surfaceAlt} !important; }
      .storefront-root .text-white { color:#fff !important; }
      .storefront-root .bg-white\/80, .storefront-root .bg-white\/90, .storefront-root .bg-white\/95 { background:${surface}f2 !important; }
      .storefront-root .border-white { border-color:${border} !important; }
    ` : ''}
    ${theme.id === 'editorial-noir' ? `
      .storefront-root #home-hero-header > div:first-child { height:220px !important; }
      .storefront-root #home-products-section > div:last-child { gap:10px !important; }
      .storefront-root #home-products-section > div:last-child > div { border-radius:4px !important; padding:3px !important; }
    ` : ''}
    ${theme.id === 'street-archive' ? `
      .storefront-root { background:#080809 !important; }
      .storefront-root #home-hero-header { background:#080809 !important; color:#fff !important; }
      .storefront-root #home-products-section .bg-white { background:#151517 !important; }
      .storefront-root #home-products-section > div:last-child > div { border-color:#343438 !important; }
    ` : ''}
    ${theme.id === 'cobalt-club' ? `
      .storefront-root #home-hero-header { border-bottom:3px solid ${theme.accent} !important; }
      .storefront-root #home-products-section > div:last-child > div { border-radius:16px !important; }
    ` : ''}
    ${theme.id === 'cherry-pop' ? `
      .storefront-root #home-hero-header { background:linear-gradient(180deg,${theme.accentSoft} 0%,${surface} 65%) !important; }
      .storefront-root .store-accent-bg { box-shadow:0 8px 18px ${theme.accent}30 !important; }
    ` : ''}
    ${theme.id === 'earth-utility' ? `
      .storefront-root #home-hero-header { border-top:6px solid ${theme.accent} !important; }
      .storefront-root #home-products-section > div:last-child > div { border-radius:5px !important; }
    ` : ''}
    ${theme.id === 'sage-atelier' ? `
      .storefront-root #home-hero-header { background:${theme.surfaceAlt} !important; }
      .storefront-root #home-products-section > div:last-child > div { box-shadow:0 8px 24px rgba(55,75,45,.08) !important; }
    ` : ''}
    ${theme.id === 'soft-studio' ? `
      .storefront-root #home-hero-header { background:${theme.background} !important; }
      .storefront-root #home-products-section > div:last-child > div { box-shadow:0 10px 26px rgba(120,80,45,.08) !important; }
    ` : ''}
    ${theme.id === 'gallery-minimal' ? `
      .storefront-root #home-products-section > div:last-child > div { box-shadow:none !important; border-width:1px !important; }
      .storefront-root #home-hero-header { border-bottom:1px solid ${border} !important; }
    ` : ''}
  `;
...
export function extractLogoTheme(source?: string | null, themeId?: string | null): Promise<StorefrontTheme> {
  ...
  const cacheKey = `${window.location.pathname}|${source}`;
